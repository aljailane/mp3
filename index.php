...
$add="<style>.sort{display:none}</style>";
	$title = $lang->Welcome;
	// updates

	$updates = "<div class='title'>$lang->updates </div>";
	...
	if($up_data){
		...
		$t=tsince($udata->time,$lang->time_v);
			$updates .= sprintf("<div class='update_l'>$udata->text</div>");
		}
		$updates .="<div class='more'><a href='$set->url/updates/2.html'>[..المزيـــد]</a></div>";
	}else
...
if($total_results > 0) {
	...
	foreach($data as $d){
		...
		if(is_dir(".".$d->path)){
			...
			$folders .= "<div class='content".(++$j%2==0 ? "2" : "")."'><img class='f_icon' src='$f_icon' alt='&#187;'/>
			<a href='$set->url/data/$d->id/d.html'>$d->name </a>
<br>
عدد المحتويات
 [<font color='red'>$count</font>]
<br>
".sprintf($_admin,$d->id)." </div>";
			...
			<a href='$set->url/data/file/$d->id/d.html'>";
			...
			if($d->icon == '') {
				...
				if(in_array($ext->extension,array('png','jpg','jpeg','gif','mp3'))) {
			...
			$w_mark=" | <b style='color:green'>علامة مائية</b> | ";
			}
			}
			else{
			$w_mark="";
			...
			$files .= "".$d->name." $new_text</a><br/>".convert($d->size)." ك.ب| $d->views الزيارات $w_mark".sprintf($_admin,$d->id)." </div>";
