...
function mp3tags_show($value) {
    ...
    if ($file AND $set->plugins['show_tags'] == 1 AND file_exists(".".$file->path)) {
        ...
        if ($tags['file_format'])
            $mp3tagsInfo .= "<div class='a5".(++$i % 2 == 0 ? 2 : "")."'>$lang->tag_fileformat : ".htmlentities($tags['file_format'])."</div>";
        if ($tags['mime_type'])
        ...
        if ($ext->extension == "mp3") {
            ...
            if (isset($_GET['preview'])) {
                ...
                if ($_GET['name']) {
                    ...
                    $zipInfo = "<div class='title'>قائمة الملفات من <span style='color:green;'>$file->name</span></div>";
                    for ($z = $start; $z < $end; $z++) {
                        if ($lists[$z])
                ...
                $zipInfo .= "<div class='a3'><= <a href='$set->url/data/file/$file->id/d.html'>$lang->back_to $file->name</a></div>";
                $value = $zipInfo;
            }
            else {
